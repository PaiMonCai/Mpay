...
Route::group('/api/cashier', function () {
    ...
    Route::get('/identity/alipay-callback', [CashierController::class, 'identityAlipayCallback'])->name('cashierIdentityAlipayCallback')->setParams(['real_name' => '收银台支付宝生活号授权回调']);
    Route::get('/identity/unionpay-callback', [CashierController::class, 'identityUnionpayCallback'])->name('cashierIdentityUnionpayCallback')->setParams(['real_name' => '收银台云闪付用户授权回调']);
...
// 支付订单接口。
    Route::group('/pay', function () {
        ...
        Route::post('/refund/{refundNo}/callback', [EpayV2Controller::class, 'refundCallback'])->name('epayRefundCallback')->setParams(['real_name' => '退款渠道回调']);
