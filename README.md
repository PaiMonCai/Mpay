...
| 📌 宝塔面板`Docker`安装教程 | 使用 Docker 发行版一键部署 MPAY V2 服务 | 视频教程即将发布 |
...
本发行版已加入 Docker Compose 与内置支付宝 OpenAPI 账单 watcher。推荐先阅读 [`DOCKER_DEPLOY.md`](DOCKER_DEPLOY.md)。
...
默认仅监听 `127.0.0.1:8787`，请使用 Nginx/OpenResty/1Panel 配置 HTTPS 反向代理。首次访问 `/install`，数据库 Host 填 `mysql`，Redis Host 填 `redis`。
