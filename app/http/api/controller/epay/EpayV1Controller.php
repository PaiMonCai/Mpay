...
class EpayV1Controller extends BaseController
{
    ...
    public function submit(Request $request): Response
    {
        ...
        try {
            ...
            if ((int) ($requestPayload['pid'] ?? 0) > 0) {
                Limiter::check('epay-v1-submit-merchant:' . (int) $requestPayload['pid'], 60, 60, '商户接口请求过于频繁，请稍后再试');
            }
            $payload = $this->validated($requestPayload, EpayV1Validator::class, 'submit');

            return $this->epayV1ProtocolService->submit($payload, $requestPayload, $request);
        } catch (Throwable $e) {
            return $this->epayV1ProtocolService->entryErrorResponse($requestPayload, $e);
        }
    }
    ...
    public function mapi(Request $request): Response
    {
        ...
        try {
            ...
            if ((int) ($requestPayload['pid'] ?? 0) > 0) {
                Limiter::check('epay-v1-mapi-merchant:' . (int) $requestPayload['pid'], 60, 60, '商户接口请求过于频繁，请稍后再试');
            }
            $payload = $this->validated($requestPayload, EpayV1Validator::class, 'mapi');

            return json($this->epayV1ProtocolService->mapi($payload, $requestPayload, $request));
        } catch (Throwable $e) {
            return json($this->epayV1ProtocolService->failureResponse($e));
        }
    }
    ...
    public function api(Request $request): Response
    {
        ...
        try {
            ...
            if ($scene === null) {
                return json($this->epayV1ProtocolService->failureResponse(new \InvalidArgumentException('不支持的操作类型')));
            }
