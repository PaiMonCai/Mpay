...
class EpayMockChainTest extends Command
{
    ...
    private function runV1Chain(OutputInterface $output, PaymentType $paymentType): array
    {
        ...
        $submitBusinessPayload = $submitPayload;
        unset($submitBusinessPayload['clientip'], $submitBusinessPayload['sitename']);
        $submitResponse = $this->resolve(EpayV1ProtocolService::class)->submit(
            $submitBusinessPayload,
            $submitPayload,
            $this->buildRequest($submitPayload, '/submit.php')
        );
        ...
        $mapiBusinessPayload = $mapiPayload;
        unset($mapiBusinessPayload['sitename']);
        $mapiResponse = $this->resolve(EpayV1ProtocolService::class)->mapi(
            $mapiBusinessPayload,
            $mapiPayload,
            $this->buildRequest($mapiPayload, '/mapi.php')
        );
