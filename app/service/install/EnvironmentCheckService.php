...
class EnvironmentCheckService extends BaseService
{
    ...
    public function check(): array
    {
        $items = [
            ...
            $this->writableFileItem('.env 可写或可创建', base_path(false) . DIRECTORY_SEPARATOR . '.env'),
        ];
     ...
     * 构建文件可写检测项。
     *
     * Docker 发行版会把 /app/.env 链接到持久化配置目录，因此当文件或
     * 符号链接已经存在时应检查该目标是否可写，而不是要求整个项目根目录可写。
     *
     * @param string $name 检测名称
     * @param string $path 文件路径
     * @return array<string, mixed> 检测项
     */
    private function writableFileItem(string $name, string $path): array
    {
